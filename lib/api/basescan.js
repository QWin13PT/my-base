 ...
 * Uses eth_gasPrice RPC method as V1 gastracker is deprecated
 * Falls back to public RPC if Basescan API fails
 */
    ...
    async () => {
      return makeTrackedRequest(SERVICE, async () => {
        try {
          ...
          console.warn('⚠️ Basescan API failed, using public RPC fallback:', error.message);
          
          // Fallback to public Base RPC (no API key needed)
          ...
          if (!response.ok) {
            throw new Error('Failed to fetch gas price from public RPC');
          }
